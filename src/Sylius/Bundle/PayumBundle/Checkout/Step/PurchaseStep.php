...
class PurchaseStep extends CheckoutStep
{
    ...
    public function forwardAction(ProcessContextInterface $context)
    {
        ...
        // Si el pago ha sido "success" completamos el pago añadiendo la cantidad
        $dispatcher = new EventDispatcher();

        if ($status->isSuccess()) {
            $type = 'success';
            $msg  = 'sylius.checkout.payment_success';

            // Lanzamos el evento para actualizar el "amount" en el pago
            $this->dispatchEvent(
                SyliusCheckoutEvents::PAYMENT_COMPLETE,
        ...
        $this->get('session')->getFlashBag()->add('success', $this->get('translator')->trans('sylius.checkout.success', array(), 'flashes'));
